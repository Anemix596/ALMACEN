...
class PresupuestoController extends Controller
{
    ...
    public function asignar_fuente_fnto_pedido(){
        ...
        if(!empty($usuario)){
            ...
            $fuente = $_POST['fuente'];
            $organo = $_POST['organo'];
            $est_prog = $_POST['est_prog'];
            $consulta = Presupuesto::fuente_organo($fuente, $organo);
            foreach($consulta as $val){
                $fuente_fnto = $val->fuente_organo;
            }

            $actualizar = Presupuesto::actualizar_fuente_fnto($no_pedido, $fuente, $organo, $fuente_fnto, $est_prog, $usuario);
            if($actualizar){
                return [1, $no_pedido];
            }
            else return 0;
