...
class CotizadorController extends Controller
{
    ...
    public function guardar_datos()
    {
        ...
        if (!empty($usuario)) {
            ...
            if ($actualiza) {
                ...
                if ($modalidad == 2 || $modalidad == 3 || $modalidad == 4 || $modalidad == 5) {
                    for ($k = 0; $k < $j; $k++) {
    ...
    public function editar_datos()
    {
        ...
        if (!empty($usuario)) {
            ...
            if ($actualiza) {
                ...
                if ($mod != $modalidad && ($mod == 2 || $mod == 3 || $mod == 4 || $mod == 5) && ($modalidad == 2 || $modalidad == 3 || $modalidad == 4 || $mod == 5)) {
                    $ban = 1;
                    for ($k = 0; $k < $j; $k++) {
                ...
                if ($mod != $modalidad && ($mod == 2 || $mod == 3 || $mod == 4 || $mod == 5) && $modalidad == 1) {
                    $ban = 1;
                    for ($k = 0; $k < $j; $k++) {
        ...
        if (!empty($usuario)) {
            ...
            if ($estado == "A") {
                ...
                if ($fecha_cot >= $fecha) {
                    ...
                    for ($j = 0; $j < $i; $j++) {
                        $dato[2][$j] = round($dato[2][$j] / $dato[4][$j], 6);
                    }
    ...
    public function ver_imprimir_solicitud_cotizacion()
    {
        ...
        if (!empty($usuario)) {
            ...
            $html = "https://repped.uatf.edu.bo/pentaho/api/repos/%3Apublic%3Apedidos%3Apedido_cotizacion.prpt/generatedContent?userid=user&password=password&output-target=pageable%2Fpdf&no_pedidoe=" . $no_pedido . "&usuarioe=" . $usuario;
            return $html;
        }
    }
    ...
    public function ver_imprimir_cuadro()
    {
        ...
        if (!empty($usuario)) {
            ...
            $html = "https://repped.uatf.edu.bo/pentaho/api/repos/%3Apublic%3Apedidos%3Acuadro_comparacion.prpt/generatedContent?userid=user&password=password&output-target=pageable%2Fpdf&pedidoe=" . $no_pedido . "&usuarioe=" . $usuario . "&solicitude=" . $no_orden;
            return $html;
        }
    }
        ...
        if (!empty($usuario)) {
            ...
            if (!empty($consulta)) {
                ...
                if ($no_contrato == "VACIO") {
                    ...
                    $html = "https://repped.uatf.edu.bo/pentaho/api/repos/%3Apublic%3Apedidos%3Aorden_compra.prpt/generatedContent?userid=user&password=password&output-target=pageable%2Fpdf&no_pedidoe=" . $no_pedido . "&usuarioe=" . $usuario . "&no_ordene=" . $no_orden;
    ...
    public function ver_imprimir_recepcion()
    {
        ...
        if (!empty($usuario)) {
            ...
            if ($contrato == "VACIO") {
                ...
                $html = "https://repped.uatf.edu.bo/pentaho/api/repos/%3Apublic%3Apedidos%3Anota_recepcion.prpt/generatedContent?userid=user&password=password&output-target=pageable%2Fpdf&usuarioe=" . $usuario . "&no_ordene=" . $no_orden;
            } else {
                ...
                $html = "https://repped.uatf.edu.bo/pentaho/api/repos/%3Apublic%3Apedidos%3Anota_recepcion_contrato.prpt/generatedContent?userid=user&password=password&output-target=pageable%2Fpdf&usuarioe=" . $usuario . "&no_ordene=" . $no_orden;
            }
            return $html;
        }
    }
    ...
    public function ver_imprimir_recepcion2()
    {
        ...
        if (!empty($usuario)) {
            ...
            $html = "https://repped.uatf.edu.bo/pentaho/api/repos/%3Apublic%3Apedidos%3Anota_recepcion_contrato_lic_exc.prpt/generatedContent?userid=user&password=password&output-target=pageable%2Fpdf&usuarioe=" . $usuario . "&no_ordene=" . $no_orden;
            return $html;
        }
    }
    ...
    public function proveedor_compra()
    {
        $usuario = session('id');
        if (!empty($usuario)) {
            return view('cotizador.modalidad.compra');
        }
    }

    public function lista_pedido_compra()
    {
        $usuario = session('id');
        if (!empty($usuario)) {
            $vector = array();
            ...
            $consulta = Cotizador::listar_pedidos_pendientes_cotizador6($usuario);
            if (count($consulta) > 0) {
                foreach ($consulta as $val) {
                    $consulta2 = Cotizador::listar_pedidos_articulos_pendientes_cotizador($val->no_pedido, $usuario);
                    if (count($consulta2) > 0) {
                        foreach ($consulta2 as $val2) {
                            $id_solicitud = $val2->id_solicitud;
                        }
                    }

                    $consulta3 = Cotizador::estado_prov_nacional_compra($val->no_pedido, $usuario);


                    if (count($consulta2) == count($consulta3)) {

                        $i = 0;
                        if (count($consulta3) > 0) {
                            foreach ($consulta3 as $val3) {
                                $estado_cot[$i] = $val3->estado_cot;
                                $i++;
                            }
                        }
                        ...
                        if (count($consulta3) > 0) {
                            foreach ($consulta3 as $val3) {
                                $estado_orden[$i] = $val3->estado_orden;
                                $i++;
                            }
                        }
                        ...
                        if (in_array("P", $estado_cot)) {
                            $c1 = 1;
                        }
                        if (in_array("P", $estado)) {
                            $c2 = 1;
                        }
                        if (in_array("P", $estado_orden)) {
                            $c3 = 1;
                        }
    ...
    public function lista_pedido_articulo_compra()
    {
        $vector = array();
        $bandera = 1;
        $no_pedido = $_POST['no'];
        ...
        if (!empty($usuario)) {
            ...
            if (count($consulta) > 0) {
                foreach ($consulta as $val) {
                    $consulta2 = Cotizador::listar_prov_nacional_compra($val->id_solicitud, $val->ide, $usuario);
                    if (count($consulta2) > 0) {
                        foreach ($consulta2 as $val2) {
                            $fecha = $val2->fechaa;
                            $id_cot = $val2->ida;
                            $fecha2 = $val2->fech;
                            $check = ($val2->precioua == null && $val2->cumplea == null ? "SI" : "NO");
                        }
                        $vector[] = array(
                            "valor" => '39',
                            "val" => '',
    ...
    public function recuperar_datos_almacenados()
    {
        ...
        if (!empty($usuario)) {
            ...
            for ($j = 0; $j < $k; $j++) {
                for ($l = 0; $l < $numero; $l++) {
                    if ($valor[$j] == $l) {
                        $datos[0][$m] = $dato[0][$valor[$j]];
                        $datos[1][$m] = $dato[1][$valor[$j]];
                        $datos[2][$m] = $dato[2][$valor[$j]];
                        $datos[3][$m] = $dato[3][$valor[$j]];
                        $datos[4][$m] = $dato[4][$valor[$j]];
                        $datos[5][$m] = $dato[5][$valor[$j]];
                    }
                }
                $m++;
            }

            return [1, $no_pedido, $id_solicitud, $datos, $m];
        }
    }
    ...
    public function guardar_datos_compra_descargar()
    {
        $usuario = session('id');
        if (!empty($usuario)) {
            $no_pedido = $_POST['no_pedido'];
            ...
            for ($j = 0; $j < $i; $j++) {
                $dato[3][$j] = round($dato[3][$j] / $dato[4][$j], 6);
            }


            for ($j = 0; $j < $i; $j++) {
                $actualizar = Cotizador::actualizar_prov_nacional($fecha_cot, $id_proveedor, $dato[3][$j], "SI", $dato[1][$j], $oferta, $tiempo);
            }
            ...
            for ($m = 0; $m < $k; $m++) {
                $actualizar_pn = Cotizador::actualizar_estado_prov_nacional($recu[0][$m], "A"); //actualiza control_ord="A", control_cot="A", estado=valor_enviado
            }
    ...
    public function lista_pedido_compra_valor()
    {
        $usuario = session('id');
        if (!empty($usuario)) {
            $vector = array();
    ...
    public function ver_imprimir_orden_compra()
    {
        $usuario = session('id');
        $html = "";
        if (!empty($usuario)) {
    ...
    public function recepcion_compra()
    {
        $usuario = session('id');
        if (!empty($usuario)) {
            return view('cotizador.recepcion.compra');
        }
    }
    ...
    public function listar_todo_orden_compra()
    {
        ...
        $usuario = session('id');
        if (!empty($usuario)) {
            $consulta = Cotizador::listar_pedido_orden_compra($usuario);
            if (count($consulta) > 0) {
                foreach ($consulta as $val) {
                    $fecha_co = explode("-", $val->fecha_orden);
                    $fecha_cot = $fecha_co[2] . "/" . $fecha_co[1] . "/" . $fecha_co[0];
    ...
    public function listar_todo_orden_valor_compra()
    {
