...
class SolicitanteController extends Controller
{
    ...
    public function recuperar_unidad_solicitante()
    {
        $usuario = session('id');
        if (!empty($usuario)) {
            $lista = Solicitante::listar_unidad_solicitante();
            $html = '';
            if (count($lista) > 0) {
                $html .= "<option value='' disabled selected>" . 'Seleccionar Unidad' . "</option>";
    ...
    public function recuperar_superior_responsable()
    {
        $usuario = session('id');
        if (!empty($usuario)) {
            $lista = Solicitante::listar_funcionarios();
            $html = '';
            if (count($lista) > 0) {
    ...
    public function ver_imprimir_pedido()
    {
        ...
        if (!empty($usuario)) {
            ...
            $html = "http://localhost:8081/pentaho/api/repos/%3Apublic%3ASteel%20Wheels%3AReports%3Asolicitante.prpt/generatedContent?userid=user&password=password&output-target=pageable%2Fpdf&aliase=" . $usuario . "&no_pedidoe=" . $no_pedido;
            return $html;
        }
    }
