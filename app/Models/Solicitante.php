...
class Solicitante extends Model
{
    ...
    public static function listar_unidad_solicitante($ci){
        $query=DB::select("SELECT * FROM listar_unidad_solicitante('$ci')");
        return $query;
    }
    ...
    public static function listar_funcionarios($id){
        $query=DB::select("SELECT * FROM listar_funcionarios('$id')");
        return $query;
    }

    public static function listar_funcionarios2($id){
        $query=DB::select("SELECT * FROM listar_funcionarios2('$id')");
        return $query;
    }
    ...
    public static function ver_lista_activos(){
        $query=DB::select("SELECT cod_mat,  (select descrip from categoria where articulo.categ_id = id) as partida, descrip,  (select descrip from unidad where articulo.unid_id = id) as unidad
        from articulo 
         where categ_id in (select id from categoria where articulo.categ_id = id)  and categ_id <> 0
        order by partida_id");
        return $query;
    }

    public static function ver_lista_suministros(){
        $query=DB::select("SELECT cod_mat,  (select descrip from part_presup where articulo.partida_id = id) as partida, descrip,  (select descrip from unidad where articulo.unid_id = id) as unidad
        from articulo 
        where id in (select articulo_id from articulo_det where articulo.id = articulo_id and cant_dispo > 0) and partida_id <> 19
        order by partida_id");
        return $query;
    }
    ...
    public static function ver_lista_suministros_valor($valor1, $valor2){
        if($valor1 == "C" && $valor2 != "S"){
            $query=DB::select("SELECT cod_mat,  (select descrip from part_presup where articulo.partida_id = id) as partida, descrip,  (select descrip from unidad where articulo.unid_id = id) as unidad
            from articulo 
            where id in (select articulo_id from articulo_det where articulo.id = articulo_id and cant_dispo > 0) and partida_id <> 19
            order by partida_id");
        ...
        else if($valor2 == "S" && $valor1 != "C"){
            $query=DB::select("SELECT cod_mat,  (select descrip from part_presup where articulo.partida_id = id) as partida, descrip,  (select descrip from unidad where articulo.unid_id = id) as unidad
            from articulo 
            where id not in (select articulo_id from articulo_det where articulo.id = articulo_id and cant_dispo > 0) and partida_id <> 19
            order by partida_id");
            return $query;
        }
        else{
            ...
            where id in (select articulo_id from articulo_det where articulo.id = articulo_id and cant_dispo > 0) and partida_id <> 19
            UNION 
            SELECT cod_mat,  (select descrip from part_presup where articulo.partida_id = id) as partida, descrip,  (select descrip from unidad where articulo.unid_id = id) as unidad
            from articulo 
            where id not in (select articulo_id from articulo_det where articulo.id = articulo_id and cant_dispo > 0) and partida_id <> 19
            order by partida");
            return $query;
