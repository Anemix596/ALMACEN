        ...
        <div class="modal fade" id="modal-without-animation3" role="dialog" style="overflow-y: scroll">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('finalizar.recepcion2') }}" id="finalizar_rec" method="POST">
                           @csrf
                            <input type="hidden" id="ordenf" name="ordenf" value="">
                            <div class="alert alert-danger m-b-0">
                                <h5><i class="fa fa-info-circle"></i> <b>¿Desea Finalizar los cambios de Nota de Recepción del Contrato Nº <label id="ordenv"></label> ?</b></h5>
...
<script>
    function imprimir_nota_recepcion(dato){
        $.ajax({
            type:'POST',
            url:'{{ route('ver.imprimir.recepcion.almacen.lic_exc') }}',
    ...
    $(function(){
        $('#guardar_datos').on("submit", function(e){
            e.preventDefault();
                ...
                $.ajax({
                    url:"{{route('guardar.datos.recepcion.licitacion.almacen')}}",
                    type:"POST",
                    datatype: "html",
                    data: formData,
                    cache:false,
...
<script>
    $(document).ready( function () {
        $('#lista').DataTable({
            pageLength:25,
            "ajax": {
                url: "{{route('listar.todo.orden.licitacion.almacen')}}",
                type: 'GET'
            },
            columns: [
...
<script>
    function ver(dato){
        ...
        $.ajax({
            type:'POST',
            url:'{{ route('listar.articulo.orden.licitacion.almacen2') }}',
        ...
        $('#lista2').DataTable({
            ...
            "ajax": {
                type:'POST',
                url:"{{ route('listar.articulo.orden.licitacion.almacen') }}",
                data:{ _token: '{{ csrf_token ()}}', no:no}
            },
            columns: [
                        {data: 'numero'  , name: 'numero'},
...
<script>
    function filtrar(dato1, dato2){
        ...
        $('#lista').DataTable({
            ...
            "ajax": {
                type:'POST',
                url:'{{ route('listar.todo.orden.licitacion.almacen.valor') }}',
