...
@section('title', 'Lista de Items - Suministros')
...
@section('content')
    ...
    <h1 class="page-header">Lista de Items - Suministros </h1>
    ...
    <div>
            <form class="form-horizontal" id="filtrar" data-parsley-validate="true" name="demo-form">
                <div class="col-md-9">
                    ...
                    <div class="checkbox checkbox-css is-valid">
                        <input type="checkbox" name="ped_aprobado" id="ped_aprobado" value="1" onchange="con_stock(this.value);"/>
                        <label for="ped_aprobado">Items Con Stock</label>
                    </div>
                    <div class="checkbox checkbox-css is-invalid">
                        <input type="checkbox" name="ped_rechazado" id="ped_rechazado" value="2" onchange="sin_stock(this.value);"/>
                        <label for="ped_rechazado">Items Sin Stock</label>
                    </div>
                </div>
            </form>
        </div>
        ...
        <div class="panel panel-inverse">
            ...
            <div class="panel-heading">
                <h4 class="panel-title">LISTA DE ITEMS - SUMINISTROS</h4>
            </div>
...
<script>
    ...
    function con_stock(dato){
        let dp = dato;
        if(ped_aprobado.checked) valor1=dp;
        else valor1=0;
        filtrar(valor1, valor2);
    }

    function sin_stock(dato){
        let dp = dato;
        if(ped_rechazado.checked) valor2=dp;
        else valor2=0;
        filtrar(valor1, valor2);
    }
</script>
...
<script>
    function filtrar(dato1, dato2){
        ...
        $('#lista').DataTable({
            ...
            "ajax": {
                type:'POST',
                url:'{{ route('ver.lista.suministros.valor') }}',
...
<script>
    $(document).ready( function () {
        $('#lista').DataTable({
            pageLength:100,
            "ajax": {
                url: "{{route('ver.lista.suministros')}}",
                type: 'GET'
            },
            columns: [
                        {data:'numero'  , name: 'numero'}, 
