    ...
    <div class="col-xl-12">
            ...
            <form class="form-horizontal" id="filtrar" data-parsley-validate="true" name="demo-form">
                <div class="col-md-9">
                    ...
                    <div class="checkbox checkbox-css">
                        ...
                        <label for="ped_pendiente">Pedidos Pendientes para su Cotización</label>
                    </div>
                    <div class="checkbox checkbox-css is-valid">
                        ...
                        <label for="ped_aprobado">Pedidos Aceptados para su Cotización</label>
                    </div>
                </div>
            </form>
        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="guardar_datos" method="POST">
                           @csrf
                           <input type="hidden" name="valor" id="valor" value="A">
                           <div class="form-group row m-b-10">
                            <label class="col-lg-3 text-lg-right col-form-label">Tipo de Modalidad <span class="text-danger">*</span> :</label>
                            <div class="col-lg-9 col-xl-6">
                                <div class="row row-space-6">
                                    <div class="col-9">
                                        <select id="modalidad" name="modalidad" class="default-select2 form-control" data-parsley-group="step-1" data-size="10" data-live-search="true" required>
                            ...
                            <br>
                            <div>
                                <button class="btn btn-success float-right" id="aceptar">Aceptar</button>
                            </div>
                        </form>
                    </div>
        ...
        <div class="modal" id="modal-without-animation2" role="dialog" style="overflow-y: scroll">
            <div class="modal-dialog" style="max-width: 60% !important;">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Editar Modalidad de Adquisición del Pedido</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body">
                        <form id="editar_datos" method="POST">
                           @csrf
                           <input type="hidden" name="evalor" id="evalor" value="A">
                           <input type="hidden" name="emod" id="emod" value="">
                           <div class="form-group row m-b-10">
                            <label class="col-lg-3 text-lg-right col-form-label">Tipo de Modalidad <span class="text-danger">*</span> :</label>
                            <div class="col-lg-9 col-xl-6">
                                <div class="row row-space-6">
                                    <div class="col-9">
                                        <select id="emodalidad" name="emodalidad" class="default-select2 form-control" data-parsley-group="step-1" data-size="10" data-live-search="true" required>
                            ...
                            <br>
                            <div>
                                <button class="btn btn-success float-right" id="actualizar">Actualizar</button>
                            </div>
                        </form>
                    </div>
                    <div class="modal-footer">
...
<script>
    function mostrar_modalidad(){
        $.ajax({
            type:'GET',
            url:'{{ route('recuperar.modalidad') }}',
            data:$(this).serialize(),
...
<script>
    function imprimir_solicitud_cotizacion(dato){
        $.ajax({
            type:'POST',
            url:'{{ route('ver.imprimir.solicitud.cotizacion.cotizador') }}',
    ...
    $(function(){
        $('#guardar_datos').on("submit", function(e){
            e.preventDefault();
                ...
                $.ajax({
                    url:"{{route('guardar.datos.cotizador')}}",
                    type:"POST",
                    datatype: "html",
                    data: formData,
                    cache:false,
                    ...
                    if(html[0] == 1){
                        ...
                        swal({ icon: "success", title: "Solicitud Aceptada"});
        ...
        $('#editar_datos').on("submit", function(e){
            e.preventDefault();
                ...
                formData.append("dato", "valor");
                $.ajax({
                    url:"{{route('editar.datos.cotizador')}}",
                    type:"POST",
                    datatype: "html",
                    data: formData,
                    cache:false,
                    ...
                    if(html[0] == 1){
                        $('#lista').DataTable().ajax.reload();
                        swal({ icon: "success", title: "Solicitud Actualizada"});
...
<script>
    $(document).ready( function () {
        $('#lista').DataTable({
            pageLength:25,
            "ajax": {
                url: "{{route('lista.pedido.pendiente.cotizador')}}",
                type: 'GET'
            },
            columns: [
...
<script>
    function ver2(dato){
        ...
        $.ajax({
            type:'POST',
            url:'{{ route('listar.pedido.articulo.pendiente.cotizador2') }}',
            data:{ _token: '{{ csrf_token ()}}', no:no},
            success:function(html){
                let id = html;
                $.ajax({
                    type:'POST',
                    url:'{{ route('mostrar.modalidad') }}',
                    data:{ _token: '{{ csrf_token ()}}', id:id},
                    success:function(html){
                        $('#emodalidad').html(html);
                    }
...
<script>
    function filtrar(dato1, dato2){
        ...
        $('#lista').DataTable({
            ...
            "ajax": {
                type:'POST',
                url:'{{ route('lista.pedido.valor.cotizador') }}',
