    ...
    <div class="col-xl-12">
        ...
        <div class="panel panel-inverse">
            ...
            <div class="panel-heading">
                <h4 class="panel-title">LISTA DE PEDIDOS PARA SU RECEPCIÓN</h4>
            </div>
                        ...
                        <form action="{{ route('finalizar.recepcion') }}" id="finalizar_rec" method="POST">
                           @csrf
                            ...
                            <div class="alert alert-danger m-b-0">
                                <h5><i class="fa fa-info-circle"></i> <b>¿Desea Finalizar los cambios de Nota de Recepción de la Nº Orden / Nº Contrato <label id="ordenv"></label> ?</b></h5>
...
<script>
    function imprimir_nota_recepcion(dato){
        $.ajax({
            type:'POST',
            url:'{{ route('ver.imprimir.recepcion.cotizador') }}',
    ...
    $(function(){
        $('#guardar_datos').on("submit", function(e){
            e.preventDefault();
                ...
                $.ajax({
                    url:"{{route('guardar.datos.recepcion')}}",
                    type:"POST",
                    datatype: "html",
                    data: formData,
                    cache:false,
...
<script>
    $(document).ready( function () {
        $('#lista').DataTable({
            pageLength:25,
            "ajax": {
                url: "{{route('listar.todo.orden.compra')}}",
                type: 'GET'
            },
            columns: [
...
<script>
    function ver(dato){
        ...
        $.ajax({
            type:'POST',
            url:'{{ route('listar.articulo.orden2') }}',
...
<script>
    function filtrar(dato1, dato2){
        ...
        $('#lista').DataTable({
            ...
            "ajax": {
                type:'POST',
                url:'{{ route('listar.todo.orden.valor.compra') }}',
