...
<script>
    function imprimir_nota_recepcion(dato){
        $.ajax({
            type:'POST',
            url:'{{ route('ver.imprimir.recepcion.cotizador.lic_exc') }}',
    ...
    $(function(){
        $('#guardar_datos').on("submit", function(e){
            e.preventDefault();
                ...
                $.ajax({
                    url:"{{route('guardar.datos.recepcion.licitacion')}}",
                    type:"POST",
                    datatype: "html",
                    data: formData,
                    cache:false,
...
<script>
    $(document).ready( function () {
        $('#lista').DataTable({
            pageLength:25,
            "ajax": {
                url: "{{route('listar.todo.orden.licitacion')}}",
                type: 'GET'
            },
            columns: [
...
<script>
    function ver(dato){
        ...
        $.ajax({
            type:'POST',
            url:'{{ route('listar.articulo.orden.licitacion2') }}',
        ...
        $('#lista2').DataTable({
            ...
            "ajax": {
                type:'POST',
                url:"{{ route('listar.articulo.orden.licitacion') }}",
                data:{ _token: '{{ csrf_token ()}}', no:no}
            },
            columns: [
                        {data: 'numero'  , name: 'numero'},
...
<script>
    function filtrar(dato1, dato2){
        ...
        $('#lista').DataTable({
            ...
            "ajax": {
                type:'POST',
                url:'{{ route('listar.todo.orden.licitacion.valor') }}',
