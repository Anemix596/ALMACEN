                    ...
                    <div class="modal-body">
                        <form id="guardar_datos" method="POST">
                           @csrf
                            ...
                            <div class="table-responsive">
                                <label for="responsable">Responsable General: </label>
                                <select name="responsable" id="responsable" class="default-select2 form-control" data-live-search="true" onchange="valor_responsable(this);"></select> <br>
                    ...
                    <div class="modal-body">
                        <form id="editar_datos" method="POST">
                           @csrf
                            ...
                            <div class="table-responsive">
                                <label for="eresponsable">Responsable General: </label>
                                <select name="eresponsable" id="eresponsable" class="default-select2 form-control" data-live-search="true" onchange="evalor_responsable(this);"></select> <br>
...
<script>
    function listar_cotizadores(){
        $.ajax({
            type:'GET',
            url:'{{ route('listar.cotizadores') }}',
            data:$(this).serialize(),
            success:function(html){
    ...
    function elistar_cotizadores(){
        $.ajax({
            type:'GET',
            url:'{{ route('listar.cotizadores') }}',
            data:$(this).serialize(),
            success:function(html){
                $('#eresponsable').html(html);
            }
            
        });
    }

    function valor_responsable(dato){
        var valor = dato.value;
        var selects = document.getElementsByName('valor[]');
        for(var i=0; i<selects.length; i++)
            selects[i].value = valor;
    }

    function evalor_responsable(dato){
        var valor = dato.value;
        var selects = document.getElementsByName('evalor[]');
        for(var i=0; i<selects.length; i++)
            selects[i].value = valor;
    }
</script>
